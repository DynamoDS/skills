...
#
# Usage: validate_skills.sh <base-ref> [skills-dir]
...
for skill in "${changed_skills[@]}"; do
  ...
  # summary. --emit-annotations emits inline PR annotations alongside the
  # markdown report. Capture the output so we can:
  # 1. Print it to stdout (includes ::error/::warning commands for GitHub Actions)
  # 2. Write non-annotation lines to the step summary without a race condition
  skill_output=$(skill-validator check --strict --emit-annotations -o markdown "${SKILLS_DIR}/$skill/" 2>&1) || FAILED=1
  echo "$skill_output"
  echo "$skill_output" | grep -v '^::' >> "${GITHUB_STEP_SUMMARY:-/dev/null}"
done
